...
class AgentViewTests(TestCase):
    ...
    def test_run_agent_requires_login(self) -> None:
        response = self.client.post(reverse("agent:run", args=[self.competitor.pk]))
        self.assertEqual(response.status_code, 302)

    @unittest.skipIf(sys.version_info >= (3, 14), "Django 4.2 test client rendering is not fully compatible with Python 3.14.")
    def test_user_cannot_run_agent_for_other_users_competitor(self) -> None:
